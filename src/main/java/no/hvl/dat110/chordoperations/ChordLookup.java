...
public class ChordLookup {
	...
	public NodeInterface findSuccessor(BigInteger key) throws RemoteException {
		...
		boolean inRange = Util.checkInterval(key,nodeID.add(BigInteger.ONE),succID);
	...
	private NodeInterface findHighestPredecessor(BigInteger ID) throws RemoteException {
		...
		for(int i = fingerTable.size()-1;i>=0;i--){
			...
			if(finger != null){
				...
				if(stub != null){
					BigInteger fingerID = stub.getNodeID();
					// check that finger is a member of the set {nodeID+1,...,ID-1} i.e. (nodeID+1 <= finger <= key-1) using the ComputeLogic
					boolean inRange = Util.checkInterval(fingerID,nodeID.add(BigInteger.ONE),ID.subtract(BigInteger.ONE));
					if(inRange){
